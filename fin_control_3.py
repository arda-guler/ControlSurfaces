...
canard_left = canard([-17.32, -10], 0)
canard_right = canard([17.32, -10], 0)
canard_top = canard([0, 20], 0)

canards = [canard_left, canard_right, canard_top]

def calc_canard_angles(x, y, roll):
    
    canard_left = (y * math.sin(math.radians(60)) - x * math.cos(math.radians(60)) + roll)
    canard_right = (y * math.sin(math.radians(60)) + x * math.cos(math.radians(60)) - roll)
    canard_top = (x + roll)

    return canard_left, canard_right, canard_top
...
while True:
    ...
    canard_left.angle, canard_right.angle, canard_top.angle = calc_canard_angles(command_turn[0], command_turn[1], command_roll)
    ...
    # canards
    tk_canvas.create_line(space2canvas([-12.99, -7.5])[0], space2canvas([-12.99, -7.5])[1],
                          space2canvas([-38.97, -22.5])[0], space2canvas([-38.97, -22.5])[1],
                          fill="hotpink")
    ...
    tk_canvas.create_line(space2canvas(canard_left.get_pos())[0], space2canvas(canard_left.get_pos())[1],
                          space2canvas([canard_left.get_pos()[0] - canard_left.angle * math.cos(math.radians(60)), 0])[0], space2canvas([0, canard_left.get_pos()[1] + canard_left.angle * math.sin(math.radians(60))])[1],
                          fill="blue", arrow=LAST)

    tk_canvas.create_line(space2canvas(canard_right.get_pos())[0], space2canvas(canard_right.get_pos())[1],
                          space2canvas([canard_right.get_pos()[0] + canard_right.angle * math.cos(math.radians(60)), 0])[0], space2canvas([0, canard_right.get_pos()[1] + canard_right.angle * math.sin(math.radians(60))])[1],
                          fill="blue", arrow=LAST)
